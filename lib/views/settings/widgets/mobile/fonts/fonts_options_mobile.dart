import 'package:almas/config/constants/loading_keys.dart';
import 'package:almas/config/languages/extension/tr.dart';
import 'package:almas/controllers/public/system_controller.dart';
import 'package:almas/providers/private/settings_presenter.dart';
import 'package:almas/ui_related/builders/mini_loading_builder.dart';
import 'package:almas/views/settings/widgets/mobile/fonts/font_option_card_mobile.dart';
import 'package:almas/views/settings/widgets/mobile/header_options_flag_mobile.dart';
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';

class FontsOptionsMobile extends StatelessWidget {
  const FontsOptionsMobile({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final presenter = Provider.of<SettingsPresenter>(context);

    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        color: SystemHandler.theme.system,
        borderRadius: BorderRadius.circular(5),
      ),
      child: Column(
        children: [
          HeaderOptionsFlagMobile(
            label: "payment-for-any-font".tr,
            data: "4,000 ${"toman".tr}",
          ),
          const SizedBox(height: 20),
          MiniLoadingBuilder(
            loadingKey: LoadingKeys.fonts,
            child: Column(
              children: (presenter.fonts.data ?? []).map((element) {
                return FontOptionCardMobile(
                  id: presenter.font?.id,
                  model: element,
                  onSelected: (id) => presenter.changeFont(element),
                );
              }).toList(),
            ),
          ),
        ],
      ),
    );
  }
}
